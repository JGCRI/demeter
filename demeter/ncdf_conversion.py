...
class DemeterToNetcdf:
    ...
    def __init__(self,
                 ...
                 end_year: int = 2005,
                 year_interval: int = 5,
                 xmin: float = -104.700861, #float = -180,
                 xmax: float = -92.748474, #float = 180,
                 ymin: float = 25.703538, #float = -90,
                 ymax: float = 34.929809, #float = 90,
                 resolution: float = 0.00151872768, #float = 0.05,
                 regrid_resolution: float = 0.00151872768, #float = 0.05,
        ...
        #pd.DataFrame(self.longitude_list, columns=["i", "long"]).long.to_csv('/data/sweet-breul/long_list.csv')
    ...
    def process_output(self,
        ...
        for index, i in enumerate(columns):
            if i != "water.1":
                ...
                temp_lu_file.latitude = temp_lu_file.latitude#.round(6)
                temp_lu_file.longitude = temp_lu_file.longitude#.round(6)
                #pd.DataFrame(temp_lu_file.longitude).longitude.unique().to_csv('/data/sweet-breul/temp_lu_long.csv')

                df_cut = temp_lu_file[temp_lu_file['longitude'].isin(self.longitude_list)]
                #print(len(df_cut.index), len(temp_lu_file['longitude'].nunique()), len(temp_lu_file['longitude']))
