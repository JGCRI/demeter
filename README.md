...
## Overview
Demeter is an open source Python package that was built to disaggregate zonal land allocation projections for alternate futures generated by the Global Change Assessment Model (GCAM).  Projected land allocation is traditionally transferred to Earth System Models (ESMs) in a variety of gridded formats and spatial resolutions to research biophysical and biogeochemical fluxes.  The process of preparing projected land allocation for each ESM is generally conducted on a case-by-case basis which introduces error and is inefficient.  Demeter makes this process seamless by providing gridded and diagnostic land-use, land-cover change products derived directly from GCAM projections in a variety of formats and resolutions commonly utilized by ESMs.  Demeter is publically available via GitHub and has an extensible output module allowing for future ESM needs to be easily accommodated.  
